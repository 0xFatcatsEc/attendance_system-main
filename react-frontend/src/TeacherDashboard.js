...
useEffect(() => {
    ...
    if (userId) {
      fetch(`${apiUrl}/api/users/${userId}`)
        ...
        .then(data => {
          ...
          const courses = data.courses ? (typeof data.courses === 'string' ? JSON.parse(data.courses) : data.courses) : [];
          const yearLevels = data.yearLevel ? (typeof data.yearLevel === 'string' ? JSON.parse(data.yearLevel) : data.yearLevel) : [];
  ...
  useEffect(() => {
    ...
    const courses = teacherData.courses ? (typeof teacherData.courses === 'string' ? JSON.parse(teacherData.courses) : teacherData.courses) : [];
    const yearLevels = teacherData.yearLevel ? (typeof teacherData.yearLevel === 'string' ? JSON.parse(teacherData.yearLevel) : teacherData.yearLevel) : [];
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                <div style={{ padding: '15px', background: 'rgba(43,89,62,0.05)', borderRadius: '12px', border: '1px solid rgba(43,89,62,0.1)' }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--accent)' }}>
                    {teacherData?.courses ? (typeof teacherData.courses === 'string' ? JSON.parse(teacherData.courses) : teacherData.courses).length : 0}
                  </div>
                ...
                <div style={{ padding: '15px', background: 'rgba(102,201,126,0.05)', borderRadius: '12px', border: '1px solid rgba(102,201,126,0.1)' }}>
                  ...
                  <div style={{ fontSize: '20px', fontWeight: 'bold', color: 'var(--accent2)' }}>
                    {teacherData?.yearLevel ? (typeof teacherData.yearLevel === 'string' ? JSON.parse(teacherData.yearLevel) : teacherData.yearLevel).length : 0}
                  </div>
